...
#include <nemo/plugins/KuramotoLag.h>

#include "neuron_model.h"



/* Compute the phase shift induced in a single oscillator. Each oscillator can
 * have a different phase lag for its incoming synapses.
 *
 * \return theta_i = sum_j { w_ij * sin(theta_j - theta_i - alpha_i) }
 */
float
    ...
    float alpha)
{
	...
	for(unsigned sid=0; sid < indegree; ++sid){
		sum += weight[sid] * sinf(sourcePhase[sid] - targetPhase - alpha);
		...
		void* rcm_ptr)
{
	...
	const float* frequency = paramBase + 0 * paramStride;
 	...
 	const float* g_alpha = paramBase + 2 * paramStride;
    ...
    const float alpha = g_alpha[n];
		...
		float k0 = f + (sumN(weight, sourcePhase, indegree, targetPhase          , alpha)/Cmean);
		float k1 = f + (sumN(weight, sourcePhase, indegree, targetPhase+k0*0.5f*h, alpha)/Cmean);
		float k2 = f + (sumN(weight, sourcePhase, indegree, targetPhase+k1*0.5f*h, alpha)/Cmean);
		float k3 = f + (sumN(weight, sourcePhase, indegree, targetPhase+k2*h     , alpha)/Cmean);
